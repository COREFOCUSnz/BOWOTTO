...
function startRace(mode, laps) {
  ...
  GAME.state = mode === 'solo' ? 'free' : 'countdown'; GAME.cd = 3.0;
...
function raceTick(dt) {
  ...
  if (GAME.state === 'countdown') {
    const cur = Math.ceil(GAME.cd - 1e-6);
    if (cur !== GAME.cdShown) { GAME.cdShown = cur; if (cur > 0) { flash(String(cur), 900); audio.beep(660, 0.12); } }
    GAME.cd -= dt;
    if (GAME.cd <= 0) { GAME.state = 'racing'; GAME.startT = now; st.lapStart = null; flash('GO', 700); audio.beep(1320, 0.45); }
  }
