...
const TRACKS = {
  ...
  // THE ARENA: not a circuit. The engine only knows "how far along the spline, how far off it", so the arena is a
  // stadium-shaped loop with a road 16 m each side of the line: the drivable floor is the whole annulus between the
  // outer wall and a 12 m infield island, inside SpringSociety's Tron light-cycle arena (96 x 148 m). Built for
  // DESTRUCTION DERBY, drives as a bowl in any mode.
  arena: { name: 'THE ARENA', sub: 'TRON LIGHT CYCLE ARENA · DESTRUCTION DERBY', theme: 'arena', km: 0.3, roadHalf: 16, loops: [], rolls: [], jumps: [], pads: [],
    ctrl: lp => [
      [30, -24, 0], [30, -8, 0], [30, 8, 0], [30, 24, 0],
      [25.98, 39, 0], [15, 49.98, 0], [0, 54, 0], [-15, 49.98, 0], [-25.98, 39, 0],
      [-30, 24, 0], [-30, 8, 0], [-30, -8, 0], [-30, -24, 0],
      [-25.98, -39, 0], [-15, -49.98, 0], [0, -54, 0], [15, -49.98, 0], [25.98, -39, 0],
    ] },
};
...
THEMES.arena = Object.assign({}, THEMES.tron, { fog: [0x03060f, 200, 1400] });   // the Tron look, the arena's own dressing
...
const ROAD_HALF = TRACK.roadHalf || 6.0;      // 12 m wide track, except where a world says otherwise (the arena's bowl)
...
const PRIZE_DERBY_WRECK = [3000, 5000, 7000, 9000];   // Destruction Derby: per rival you wreck, plus the Versus win money if you are the last one running
...
function prizeFor(mode, pos, laps, diff, newBest) {
  ...
  if (mode === 'derby') return PRIZE_DERBY_WRECK[diff] * st.wrecks + (pos === 1 ? PRIZE[diff][0] : 0);
  return 0;
}
function payout(prize, pos) {
  career.cash += prize; career.stats.races++; career.stats.earned += prize;
  if (GAME.mode === 'versus' || GAME.mode === 'derby') { if (pos === 1) career.stats.wins++; if (pos <= 3) career.stats.podiums++; }
  careerSave();
}
careerLoad();
const cloud = {
...
// The sky dome (r 8500), the stars (r 8000) and the sun disc (7800 m out) are meant to be at infinity, but they
// were pinned to the world origin while the camera drives up to 3.4 km away -- so their FAR side sat up to 11.9 km
// from the eye, past the 9000 m far plane, and got sliced off: a hole of flat background colour in the sky, worst
// on the starfield worlds where the stars ARE the backdrop. They ride with the camera now, which is what "at
// infinity" means, and nothing of the sky ever reaches the far plane.
const skyFollow = [];
function skyRide() { for (const o of skyFollow) { if (!o.userData.skyBase) o.userData.skyBase = o.position.clone(); o.position.copy(o.userData.skyBase).add(camera.position); } }
...
const sunDir = new THREE.Vector3().setFromSphericalCoords(1, THREE.MathUtils.degToRad(90 - TH.sunEl), THREE.MathUtils.degToRad(35));
{
  ...
  if (DAY) { const sunDisc = new THREE.Mesh(new THREE.SphereGeometry(140, 16, 12), new THREE.MeshBasicMaterial({ color: 0xfff6d8, fog: false, toneMapped: false })); sunDisc.position.copy(sunDir).multiplyScalar(7800); sunDisc.position.y -= 200; scene.add(sunDisc); skyFollow.push(sunDisc); }
  if (false) {
    ...
    dome.position.y = -200; scene.add(dome); skyFollow.push(dome);
    if (TH.stars) {
      ...
      const stars = new THREE.Points(sg, new THREE.PointsMaterial({ color: TH.starCol || 0x9fd8ff, size: 14, sizeAttenuation: true, fog: false, toneMapped: false, transparent: true, opacity: 0.75 })); scene.add(stars); skyFollow.push(stars);
    }
  }
}
...
// THE ARENA borrows a model built for a different game, and its real wall is not a uniform ROAD_HALF offset --
// a support strut inside the same mesh as the boundary wall can sit as close as 0.35 m from the CENTRELINE at a
// couple of points on the lap. DRESS.arena measures the model's real clearance once at load and fills this in;
// every other track leaves it null and the flat D_HIT keeps doing the job.
let ARENA_LIMIT = null;
function wallLimitAt(sVal) { if (!ARENA_LIMIT) return D_HIT; return Math.min(D_HIT, ARENA_LIMIT[sampleAt(sVal).i]); }
...
const DRESS = {
  arena() {
    // the bowl's floor is the arena model's own grid, so the painted road strip goes; the neon walls at +-D_WALL stay,
    // they are the real boundary. The model is hosted-only (3 MB); the single-file page keeps the Tron grid floor
    roadMesh.visible = false;
    if (location.protocol === 'file:' || document.getElementById('revuelto-glb')) return;
    fetch('arena.glb').then(r => r.ok ? r.arrayBuffer() : null).then(b => { if (!b) return;
      new THREE.GLTFLoader().parse(b, '', g => {
        const grp = g.scene, box = new THREE.Box3().setFromObject(grp), c = box.getCenter(new THREE.Vector3());
        grp.position.set(-c.x, -box.min.y + 0.06, -c.z);   // centred on the spline's origin, floor tiles just above the grid
        grp.traverse(o => { if (o.isMesh) { o.receiveShadow = true; if (o.material && o.material.map) o.material.map.anisotropy = maxAniso; } });
        // This model was built for a light-cycle game, not for this one, and it has structures standing well
        // inside the bowl -- polySurface1 is a 15 m slab about 5.5 m off the racing line. The engine's drivable
        // area is the whole annulus and the physics knows nothing about the model, so you drive straight into
        // them: the car disappears inside the slab and the chase camera ends up in there too. A first version of
        // this check sampled vertex heights, which missed it -- a plain box's vertices sit only at its top and
        // bottom corners, never at car height in between, even though the solid box obviously spans through it.
        // Test the BOUNDING BOX instead: does its y-range reach car height, and does its horizontal footprint
        // (all 8 corners, so shape doesn't matter) come well inside the road rather than sitting out at the
        // boundary -- that distinction is what keeps the wall itself, which the drivable width is measured
        // against and legitimately sits close to the corridor's outer edge.
        grp.updateMatrixWorld(true);
        const LOW = 0.4, HIGH = 2.4, INSIDE = Math.max(2, ROAD_HALF - 5), gone = [];
        const corner = new THREE.Vector3();
        grp.traverse(o => {
          if (!o.isMesh || !o.geometry) return;
          const bb = new THREE.Box3().setFromObject(o); if (bb.isEmpty()) return;
          const sz = bb.getSize(new THREE.Vector3());
          if (sz.y < 2.5) return;                                    // floor, kerbs: never tall enough to block
          if (Math.max(sz.x, sz.z) > 40) return;                     // the shell, floor and big background pieces, not a prop
          if (bb.max.y < LOW || bb.min.y > HIGH) return;              // never at car height
          let near = 1e9;
          for (let cx = 0; cx < 2; cx++) for (let cz = 0; cz < 2; cz++) {
            corner.set(cx ? bb.max.x : bb.min.x, 0, cz ? bb.max.z : bb.min.z);
            for (let i = 0; i < N; i += 6) { const q = S[i], dx = corner.x - q.x, dz = corner.z - q.z, dd = dx * dx + dz * dz; if (dd < near) near = dd; }
          }
          if (Math.sqrt(near) < INSIDE) { o.visible = false; gone.push((o.name || '?').slice(0, 24)); }
        });
        if (gone.length) console.log('arena: ' + gone.length + ' obstacle mesh(es) stood in the driving corridor, hidden:', gone.join(', '));
        // The obstacle-hiding pass above skips anything wider than 40 m as "the shell, not a prop" -- but here the
        // outer wall and its internal cross-bracing are ONE mesh, so a strut that comes within 0.35 m of the
        // CENTRELINE at a couple of points on the lap rides along with a wall we still need to keep. Measure the
        // model's real clearance instead of hiding anything: raycast out from every spline sample, both ways, and
        // take the closest hit (minus the car's half-width and a margin) as the true safe half-road at that point.
        {
          const solids = []; grp.traverse(o => { if (o.isMesh && o.visible) solids.push(o); });
          if (solids.length) {
            const ray = new THREE.Raycaster(), raw = new Float32Array(N).fill(ROAD_HALF);
            for (let i = 0; i < N; i += 2) {
              const q = sampleAt(CUM[i]); let best = ROAD_HALF;
              for (const dir of [1, -1]) {
                ray.set(new THREE.Vector3(q.p.x, 3, q.p.z), new THREE.Vector3(q.b.x * dir, 0, q.b.z * dir).normalize());
                const hit = ray.intersectObjects(solids, false)[0];
                if (hit) best = Math.min(best, hit.distance - 1.7);   // 1.7 m: half the car's width plus a buffer
              }
              raw[i] = Math.max(2.4, best);
            }
            for (let i = 1; i < N; i += 2) raw[i] = raw[i - 1];   // fill the samples the loop above skipped
            const smoothed = new Float32Array(N);   // a few metres either side, so the safe width tightens gradually
            for (let i = 0; i < N; i++) { let m = Infinity; for (let k = -8; k <= 8; k++) m = Math.min(m, raw[(i + k + N) % N]); smoothed[i] = m; }
            ARENA_LIMIT = smoothed;
            const worst = Math.min(...smoothed);
            if (worst < ROAD_HALF - 0.5) console.log('arena: real wall clearance narrows to ' + worst.toFixed(1) + ' m (nominal road half is ' + ROAD_HALF + ' m) -- the drivable width now follows the model');
          }
        }
        scene.add(grp);
      }, e => console.error(e));
  ...
  docks(scatter) {
    ...
    // The road runs the length of these ships at x = 960, ramping from y = 3 up to the deck and back down. The
    // hull and deck used to be pinned at a fixed height (deck top y = 8.0) regardless, so wherever the road was
    // still below that -- the whole climb onto ship one, and most of ship two, which the road crosses at y = 6 --
    // the car was driving INSIDE a solid slab: the chase camera, 1.9 m higher, looked down on an opaque lid with
    // no car under it. Sit each ship under its own stretch of road instead, measured from the spline.
    for (const [z0, z1] of [[-1180, -1470], [-1600, -1960]]) { const L = z0 - z1, cz = (z0 + z1) / 2;
      let roadMin = Infinity;
      for (let i = 0; i < N; i++) { const q = S[i]; if (q.z <= z0 && q.z >= z1 && Math.abs(q.x - 960) < 34) roadMin = Math.min(roadMin, q.y); }
      const deckTop = (Number.isFinite(roadMin) ? roadMin : 8) - 0.35;
      const hull = new THREE.Mesh(new THREE.BoxGeometry(46, 12, L), new THREE.MeshStandardMaterial({ color: 0x1a2a3a, roughness: 0.6, metalness: 0.4 })); hull.position.set(960, deckTop - 6.5, cz); scene.add(hull);
      const deck = new THREE.Mesh(new THREE.BoxGeometry(46, 1, L), new THREE.MeshStandardMaterial({ color: 0x7a1a1a, roughness: 0.8 })); deck.position.set(960, deckTop - 0.5, cz); scene.add(deck);
      const bridge = new THREE.Mesh(new THREE.BoxGeometry(12, 14, 12), new THREE.MeshStandardMaterial({ color: 0xe8e8e8, roughness: 0.6 })); bridge.position.set(960 + 16, deckTop + 7, z0 - 20); bridge.castShadow = true; scene.add(bridge); }
...
// A model that throws halfway through fitting leaves the old body already removed and the procedural one still
// switched off -- i.e. no car at all, permanently. Wrap it: on failure, put the procedural Revuelto back so the
// player always has something to drive.
function installModel(root, name) {
  try { installModelInner(root, name); }
  catch (e) { console.error('installModel', e); if (!customModel) { procBody.visible = true; wheels.forEach(w => w.steer.visible = true); } flash('MODEL FAILED', 2200); }
}
function installModelInner(root, name) {
  ...
  if (prescaled) { wrap.rotation.y = Math.PI / 2 + customYaw; }        // converter output: nose on +Z, real size, on the ground
    ...
    // undo OUR last pass first (only ours -- userData.simJunk): car roots are cached and re-installed when you
    // switch back to a car, and bodyBounds ignores hidden meshes, so without this the measured body shrinks a
    // little every time, more of the car qualifies as "scenery", and the car eats itself switch by switch.
    root.traverse(o => { if (o.isMesh && o.userData.simJunk) { o.visible = true; o.userData.simJunk = false; } });
    const bb = bodyBounds(wrap);
    for (const o of bb.junk) { o.visible = false; o.userData.simJunk = true; }   // stage floors and fake light cones are not part of the car
...
const diffNow = () => (GAME.mode === 'versus' || GAME.mode === 'derby') ? DIFFS[GAME.diff] : DIFF_FREE;
const RIVALS = [
...
function buildRivalVisual(a) {
  ...
  for (const part of rivalTemplate.parts) {
    ...
    const m = new THREE.Mesh(part.geo, mat); m.castShadow = false; m.receiveShadow = true; m.userData.paint = part.paint; body.add(m);
  }
  a.wheels = [];
...
function makeRival(R) {
  const a = { name: R.name, hex: R.hex, skill: R.skill, lane: R.lane, s: 0, d: 0, u: 0, dv: 0, psi: 0, spinT: 0, spinDir: 1, crossings: 0, lastP: 0, finishT: null, lapStart: null, best: null,
    boostT: 0, boostCd: 0, glow: 0, hitT: 0, nos: 1, nosOn: false, attackT: 0, attack: 0, errT: 20 + Math.random() * 30, mistake: null, hp: 100, wrecked: false, wreckT: 0, pos: new THREE.Vector3(), fwd: new THREE.Vector3(1, 0, 0), up: new THREE.Vector3(0, 1, 0), right: new THREE.Vector3(), idx: 0, grp: null, body: null, wheels: [], snd: null };
...
function rivalStep(a, dt, now) {
  const i = a.idx, kap = KAPPA[i], go = GAME.state === 'racing' || GAME.state === 'finished', derby = GAME.mode === 'derby';
  ...
  a.errT -= dt;
  if (a.errT <= 0 && go && !derby && a.spinT <= 0 && !a.mistake) {
  ...
  if (a.spinT > 0) { a.spinT -= dt; a.psi += a.spinDir * 5.5 * dt; a.u = Math.max(0, a.u - 9 * dt); a.dv *= Math.max(0, 1 - 2 * dt); }
  ...
  else if (a.wrecked) { a.u = Math.max(0, a.u - 12 * dt); a.dv *= Math.max(0, 1 - 3 * dt); }   // out of the derby: rolls to a stop where it died, a wreck to drive around
  else {
    // the derby: no racing line, hunt the player. Ahead and in reach, close at their speed plus ten and line up on
    // their lane; far ahead, flat out to catch up; beside, match pace and lean on them. When the player is behind,
    // an arena loop is short enough to drive on and come round on their tail (a real derby circles), while on a
    // circuit that would mean never meeting again, so there the rival creeps in the player's lane and waits
    let lane = null;
    if (derby) {
      const shortLoop = trackLen < 1500;
      if (st.wrecked) target = Math.min(VLIM[i] * 1.1, D.vmax * 0.6);
      else if (dsP > 4 && dsP < 80) { target = Math.min(VLIM[i] * 1.3, Math.abs(st.u) + 10); lane = st.d; }
      else if (dsP >= 80) target = Math.min(VLIM[i] * 1.3, D.vmax);
      else if (dsP > -7) { target = Math.max(6, Math.abs(st.u)); lane = st.d; }
      else if (shortLoop) { target = Math.min(VLIM[i] * 1.2, D.vmax * 0.8); lane = -st.d * 0.4; }
      else { target = 5; lane = st.d; }
    }
    ...
    if (lane != null) dT = lane;
    const cars = (attacking || derby) ? [] : [{ s: st.s, d: st.d, u: st.u }]; if (!derby) for (const o of ai) if (o !== a) cars.push(o);
  ...
  { const lim = wallLimitAt(a.s); if (Math.abs(a.d) > lim) { if (derby && Math.abs(a.dv) > 3) rivalDamage(a, Math.abs(a.dv) * 0.3, false); a.d = Math.sign(a.d) * lim; a.dv = -a.dv * 0.3; a.u *= 0.9; } }
  ...
  a.body.rotation.z = damp(a.body.rotation.z, a.wrecked ? 0.08 : 0, 8, dt); a.body.rotation.x = damp(a.body.rotation.x, a.wrecked ? -0.05 : a.dv * 0.02, 8, dt);   // a wreck sits down on a flat, nose down
  a.glowDisc.material.opacity = (a.wrecked ? 0.05 : 0.45 + 0.5 * a.glow) * GLOW_K; a.glowDisc.material.color.setHex(a.nosOn ? 0xa64dff : a.hex);
}
...
function resolveContact(A, B) {
  ...
  const aggA = intoA >= intoB; contactAgg = aggA;
...
// Destruction Derby damage. resolveContact leaves who was doing the hitting in contactAgg (A moving into B); the
// hitter takes 30 % of what it deals, so ramming pays. Closing speed x 2 hp: a 22 m/s hit is 44 to the car struck,
// 13 to the one that did it -- two clean hits to wreck a rival, nine rams in the tank to do it with
let contactAgg = false;
function rivalDamage(a, dmg, byPlayer) {
  if (GAME.mode !== 'derby' || a.wrecked || GAME.state !== 'racing') return;
  a.hp -= dmg; if (a.hp > 0) return;
  a.hp = 0; a.wrecked = true; a.wreckT = performance.now(); a.nosOn = false; a.attack = 0; a.mistake = null;
  a.body.traverse(m => { if (m.isMesh && m.userData.paint) { m.material.color.setHex(0x26262b); m.material.metalness = 0.5; m.material.roughness = 0.9; m.material.envMapIntensity = 0.2; } });   // burnt out: its own paint copy, so only this car
...
function startRace(mode, laps) {
  ...
  if (mode === 'versus') {
    ...
    // the field spreads out ahead of you, alternating sides, and the moment the lights go green it turns on you
    const n = DERBY_N[GAME.diff]; placeAtS(12, 0);
    for (let k = 0; k < n; k++) { const a = makeRival(DERBY_RIVALS[k % DERBY_RIVALS.length]); a.s = (42 + 22 * k) % trackLen; a.d = (k % 2 ? 1 : -1) * Math.min(ROAD_HALF * 0.55, 3.2 + k * 0.4); a.lastP = a.s / trackLen; rivalPose(a); audio.rivalStart(a); ai.push(a); }
...
function raceTick(dt) {
  ...
  if (GAME.mode === 'derby' && GAME.state === 'racing' && !st.wrecked && ai.length && ai.every(a => a.wrecked)) finishRace(now);   // last car standing
  if (GAME.state === 'finished' && GAME.resultsAt && now > GAME.resultsAt) { GAME.resultsAt = 0; showResults(); }
}
function standings() {
  if (GAME.mode === 'derby') {   // still running first (most hp on top), then the wrecks, latest to go first
    const rows = [{ me: true, name: career.name || 'YOU', hex: PAINTS[paintIdx].hex, hp: st.hp, wrecked: st.wrecked, wreckT: st.wreckT }];
    for (const a of ai) rows.push({ me: false, name: a.name, hex: a.hex, hp: a.hp, wrecked: a.wrecked, wreckT: a.wreckT });
    rows.sort((p, q) => p.wrecked !== q.wrecked ? (p.wrecked ? 1 : -1) : p.wrecked ? q.wreckT - p.wreckT : q.hp - p.hp);
    return rows;
  }
...
function showResults() {
  ...
  if (GAME.mode === 'derby') {
    $('res-title').textContent = pos === 1 ? 'LAST CAR STANDING' : 'WRECKED · P' + pos + ' OF ' + rows.length;
    $('res-sub').textContent = 'DESTRUCTION DERBY · ' + DIFFS[GAME.diff].name + ' · ' + (rows.length - 1) + ' RIVALS · ' + fmtTime(GAME.finishT);
    $('res-cash').textContent = GAME.prize > 0 ? '+ ' + fmtCash(GAME.prize) + ' · BALANCE ' + fmtCash(career.cash) : '';
    const head = document.createElement('div'); head.className = 'row head'; head.innerHTML = '<b></b><i style="visibility:hidden"></i><span>DRIVER</span><em>DAMAGE</em><b>WRECKS</b>'; box.appendChild(head);
    rows.forEach((r, k) => { const el = document.createElement('div'); el.className = 'row' + (r.me ? ' me' : '');
      el.innerHTML = `<b>P${k + 1}</b><i style="background:#${r.hex.toString(16).padStart(6, '0')}"></i><span>${r.name}</span><em>${r.wrecked ? 'OUT' : Math.round(r.hp) + '%'}</em><b>${r.me ? st.wrecks : ''}</b>`; box.appendChild(el); });
...
function updateRaceHUD() {
  ...
  if (GAME.mode !== 'versus' && !derby) return;
  const box = $('standings');
  ...
  rows.forEach((r, k) => {
    ...
    let gap;
    if (derby) gap = r.wrecked ? 'OUT' : Math.round(r.hp) + '%';
    else if (r.finishT != null) gap = k === 0 ? 'FIN' : '+' + ((r.finishT - leader.finishT) / 1000).toFixed(1);
...
function readInput() {
  ...
  if (st.wrecked) { inp.throttle = 0; inp.brake = 1; inp.steer = 0; inp.hand = false; inp.nos = false; }   // wrecked: the car is done, it rolls to a stop
}
...
{ // the start screens: course → mode (and laps) → rivals (Versus). Choices are remembered, so a course change (which
  ...
  if (ls('revuelto.mode') && ['solo', 'time', 'versus', 'derby'].includes(ls('revuelto.mode'))) GAME.mode = ls('revuelto.mode');
  ...
  const showStep = n => {
    ...
    const last = n === 3 || (n === 2 && GAME.mode !== 'versus' && GAME.mode !== 'derby');
    ...
    $('start-btn').textContent = GAME.mode === 'versus' ? 'START RACE' : GAME.mode === 'derby' ? 'START DERBY' : GAME.mode === 'time' ? 'START TIME TRIAL' : 'START ENGINE';
  };
  ...
  const setModeBtn = m => { GAME.mode = m; lsSet('revuelto.mode', m); document.querySelectorAll('#modes button').forEach(b => b.classList.toggle('on', b.dataset.m === m)); $('lapsel').classList.toggle('hidden', m === 'solo' || m === 'derby'); showStep(2); };
  ...
  document.querySelectorAll('#modes button').forEach(b => b.classList.toggle('on', b.dataset.m === GAME.mode)); $('lapsel').classList.toggle('hidden', GAME.mode === 'solo' || GAME.mode === 'derby');
  showStep(step);
...
function step(dt) {
  ...
  const dLim = st.roof ? ROOF_W : wallLimitAt(st.s);   // on a roof the deck edge is the wall; ARENA_LIMIT tightens it where the model's own geometry sits closer than the nominal wall
...
function updateHUD() {
  ...
  if ((hudTick++ & 3) === 0) {
    ...
    { // what you are playing: mode, laps, difficulty, camera
      const parts = [GAME.mode === 'versus' ? 'VERSUS' : GAME.mode === 'derby' ? 'DESTRUCTION DERBY' : GAME.mode === 'time' ? 'TIME TRIAL' : 'SOLO'];
      if (GAME.mode !== 'solo' && GAME.mode !== 'derby') parts.push(GAME.laps + (GAME.laps === 1 ? ' LAP' : ' LAPS'));
      if (GAME.mode === 'versus' || GAME.mode === 'derby') parts.push(DIFFS[GAME.diff].name);
...
function frame(now) {
  ...
  // the car is hidden for the reflection render so it cannot reflect itself. try/finally because if cubeCam.update
  // throws (a lost context, a bad texture, a shader that failed to compile on this GPU) the old code left the car
  // invisible for the rest of the session -- the car simply disappears and never comes back.
  if ((frames & 1) === 0) {
    car.visible = false; if (mirror) mirror.visible = false; for (const a of ai) a.grp.visible = a.pos.distanceToSquared(st.pos) < 45 * 45;
    try { cubeCam.position.copy(st.pos).addScaledVector(st.up, 0.7); cubeCam.update(renderer, scene); }
    catch (e) { console.error('reflection pass', e); }
    finally { car.visible = true; if (mirror) mirror.visible = true; for (const a of ai) a.grp.visible = true; }
  }
