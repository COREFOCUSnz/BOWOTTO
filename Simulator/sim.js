...
const TRACKS = {
  ...
  sky: { name: 'STRATOS', sub: 'MONORAIL ABOVE THE CLOUDS · LED TUNNELS', theme: 'sky', km: 13.6, softWalls: true,
    loops: [], rolls: [],
    ...
    tunnels: [{ from: V3(300, 730, -1100), to: V3(-150, 715, -1450), hex: 0x2ee6ff }, { from: V3(-100, 712, -2650), to: V3(-650, 730, -2450), hex: 0xff3af0 },
              { from: V3(-1500, 740, -1100), to: V3(-1450, 740, -700), hex: 0x3aff5a }, { from: V3(-650, 740, 255), to: V3(-250, 725, 170), hex: 0xff8a2a }],
    ...
    ctrl: lp => [
      ...
      [-707, 252, 740], [-450, 250, 732], [-200, 150, 722], [-150, 80, 712], [-500, 60, 706],
      [-700, 48, 690], [-950, 25, 694], [-1250, 0, 700],
    ] },
};
...
}) : null;
{
  ...
  if (THEME === 'sky') {
    ...
    const puffs = []; for (let k = 0; k < 2600; k++) { const i = Math.floor(rnd() * N), f = frameAt(i), side = rnd() < 0.5 ? -1 : 1; const s0 = 14 + rnd() * rnd() * 70, off = 26 + s0 * 1.15 + rnd() * rnd() * 700, p = f.p.clone().addScaledVector(f.b, side * off).addScaledVector(f.t, (rnd() - 0.5) * 80); p.y += (rnd() - 0.55) * 160; puffs.push({ x: p.x, y: p.y, z: p.z, s: s0, rot: rnd() * 6.28 }); }
...
const st = {
  ...
  hits: 0, hitT: 0, boost2Cd: 0, crossings: 0, lapsDone: 0, spinT: 0, spinW: 0, boostT: 0, boostCd: 0, glow: 0, nos: 1, nosOn: false, air: false, vel: new THREE.Vector3(), airT: 0, resets: 0,
...
function step(dt) {
  ...
  if (BOOST2[fr0.i] && !st.air && st.boost2Cd <= 0 && st.u > 2) { st.u = Math.min(st.u * 1.4, 118); st.boost2Cd = 2.0; st.glow = 1; st.shake = Math.max(st.shake || 0, 0.5); audio.boost(); audio.record(); flash('SUPER BOOST +40%', 900, '#ff3af0'); }
  ...
  if (Math.abs(st.d) > D_HIT) {
    ...
    const retain = TRACK.softWalls ? 1 : clamp(1 - 0.85 * Math.sin(phi) - 0.02, 0.12, 1);   // soft walls (STRATOS) keep you on the rail without taking speed
