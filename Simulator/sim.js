...
function placeOnTrack(idx) {
  ...
  st.u = st.w = st.yaw = 0; st.air = false; st.airT = 0; st.gear = 0; st.rpm = CAR.idle; st.reverse = false; st.lapStart = null; st.halfSeen = false;
...
function step(dt) {
  ...
  if (st.air) {
    ...
    const hv = st.vel.clone(); hv.y = 0;
    const frA = sampleAt(st.s); st.s = ((st.s + hv.dot(frA.t) * dt) % trackLen + trackLen) % trackLen; st.d += hv.dot(frA.b) * dt;
