...
const cloud = {
  ...
  push() {
    ...
    this.pending = setTimeout(() => this.doc().set({ name: career.name || null, cash: career.cash, tiers: career.tiers, paints: career.paints, cars: career.cars, car: career.car, stats: career.stats, updated: career.updated, account: this.user.displayName || null }).catch(() => { this.status = 'CLOUD ERROR'; }), 800);
  },
};
...
{ // the garage: prize money spent on parts and paints, with the car turning on the photo camera behind the panel
  ...
  $('g-cars').addEventListener('click', e => { const b = e.target.closest('[data-drive],[data-buycar]'); if (!b) return; e.stopPropagation(); if (b.dataset.drive) { carSelect(b.dataset.drive, true); return; } const id = b.dataset.buycar, r = buyCar(id); if (r === 'OK') { flash('BOUGHT · LAMBORGHINI ' + CARS[id].name, 1400, '#ffd21f'); carSelect(id, true); garageRefresh(); } else if (r === 'POOR') flash('NOT ENOUGH MONEY', 1200); });
