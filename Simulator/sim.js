...
{
  const domeCols = { tron: [0x01020a, 0x0a2a55], matrix: [0x000000, 0x03200a], woods: [0x140a26, 0xff6a2a], snow: [0xb8c4d0, 0xf0f4f8], desert: [0x2a66c0, 0xe2caa8], sonic: [0x1f78e6, 0xbfe6ff], ocean: [0x0a4a86, 0x03182c] }[THEME];
...
const TERRAIN = { desert: { amp: 260, base: 20, cliff: 26, freq: 0.00075 }, sonic: { amp: 60, base: 8, cliff: 0, freq: 0.0011 }, woods: { amp: 300, base: 90, cliff: 14, freq: 0.0007 }, snow: { amp: 240, base: 100, cliff: 10, freq: 0.0008 }, ocean: { amp: 70, base: -22, cliff: 0, freq: 0.0012 } }[THEME];
function rawH(x, z) {
...
{ // tunnels: rounded-rectangle shell (superellipse) with two continuous shoulder light bands and a white ceiling strip,
  ...
  const tunnel = new THREE.Mesh(g, shellMatT); tunnel.receiveShadow = !TUBE; if (TUBE) tunnel.renderOrder = 5; scene.add(tunnel);
  ...
  for (const [i0, i1] of TUNNELS) {
    ...
    if (TUBE) { for (let i = i0 + 100; i < i1 - 50; i += 230) { const m = frameAt(i), l = new THREE.PointLight(0x7fe8ff, 1.1, 120, 1.4); l.position.copy(m.p).addScaledVector(m.n, 4.5); scene.add(l); } }
