...
{ // overhead light ribbons above both road edges: the "ceiling" that the paint reflects
  const mk = (off, y, w) => { const pos = []; for (let i = 0; i < N; i++) { const j = (i + 1) % N, ni = new THREE.Vector3(-T[i].z, 0, T[i].x), nj = new THREE.Vector3(-T[j].z, 0, T[j].x);
      for (const side of [1, -1]) { const a = S[i].clone().addScaledVector(ni, side * off), b = S[j].clone().addScaledVector(nj, side * off);
        pos.push(a.x - w, y, a.z, a.x + w, y, a.z, b.x + w, y, b.z, a.x - w, y, a.z, b.x + w, y, b.z, b.x - w, y, b.z); } }
    const g = new THREE.BufferGeometry(); g.setAttribute('position', new THREE.Float32BufferAttribute(pos, 3));
    const m = new THREE.Mesh(g, new THREE.MeshBasicMaterial({ color: 0xbfefff, side: THREE.DoubleSide, toneMapped: false, transparent: true, opacity: 0.55, fog: true })); scene.add(m); return m; };
}
// ---------------------------------------------------------------- track architecture: tunnels, gates, canyon, bridges, floating grid solids
const frameAt = i => ({ p: S[i], yaw: Math.atan2(-T[i].z, T[i].x), n: new THREE.Vector3(-T[i].z, 0, T[i].x) });
const inRanges = (i, ranges) => ranges.some(([a, b]) => i >= a && i <= b);
const TUNNELS = [[330, 430], [700, 830], [1040, 1150]];
const CANYON = [[520, 690]];
...
{ // tunnels: arched shell with three glowing seams, light rings every ~11 m, bright portal rings at both ends
  const seamTex = canvasTex(256, (ctx, s) => { ctx.fillStyle = '#070b14'; ctx.fillRect(0, 0, s, s); ctx.fillStyle = '#2ee6ff'; for (const u of [0.08, 0.5, 0.92]) ctx.fillRect(u * s - 2, 0, 4, s); ctx.fillStyle = '#0d3a55'; for (const u of [0.29, 0.71]) ctx.fillRect(u * s - 1, 0, 2, s); });
  const arc = [], SEG = 14, W = 9.5, H = 6.5;
  for (let k = 0; k <= SEG; k++) { const t = k / SEG, a = Math.PI * (1 - t); arc.push([Math.cos(a) * W, Math.max(0, Math.sin(a) * H) + (k === 0 || k === SEG ? 0 : 0)]); }   // half-ellipse from left floor to right floor
  const pos = [], uv = [];
  for (const [i0, i1] of TUNNELS) for (let i = i0; i < i1; i++) {
    const A = frameAt(i), B = frameAt(i + 1), v0 = CUM[i] / 10, v1 = (CUM[i] + S[i].distanceTo(S[i + 1])) / 10;
    for (let k = 0; k < SEG; k++) {
      const p = (f, q) => new THREE.Vector3(f.p.x + f.n.x * q[0], q[1], f.p.z + f.n.z * q[0]);
  ...
  const tunnel = new THREE.Mesh(g, new THREE.MeshStandardMaterial({ map: seamTex, emissiveMap: seamTex, emissive: 0xffffff, emissiveIntensity: 1.5, color: 0x8899aa, roughness: 0.5, metalness: 0.3, side: THREE.DoubleSide })); tunnel.receiveShadow = true; scene.add(tunnel);
  // rings
  const ringPath = new THREE.CatmullRomCurve3(arc.map(q => new THREE.Vector3(q[0], q[1] + 0.15, 0)));
  const ringGeo = new THREE.TubeGeometry(ringPath, 40, 0.12, 6, false), portalGeo = new THREE.TubeGeometry(ringPath, 40, 0.3, 8, false);
  const rings = [], portals = [];
  for (const [i0, i1] of TUNNELS) { for (let i = i0 + 3; i < i1 - 3; i += 3) { const f = frameAt(i); rings.push({ x: f.p.x, z: f.p.z, rot: f.yaw + Math.PI / 2 }); } for (const i of [i0, i1]) { const f = frameAt(i); portals.push({ x: f.p.x, z: f.p.z, rot: f.yaw + Math.PI / 2 }); } }
  instanced(ringGeo, neonMat, rings, false); instanced(portalGeo, neonWhite, portals, false);
}
{ // gate frames over the road between the tunnels, alternating cyan / white, some doubled
  const post = new THREE.BoxGeometry(0.6, 11, 0.6).translate(0, 5.5, 0), beam = new THREE.BoxGeometry(0.6, 0.6, 24).translate(0, 11, 0);
  const frame = mergeGeos([post.clone().translate(0, 0, -12), post.clone().translate(0, 0, 12), beam]);
  const gates = [];
  for (let i = 40; i < N; i += 36) { if (inRanges(i, TUNNELS) || inRanges(i, CANYON)) continue; const f = frameAt(i); gates.push({ x: f.p.x, z: f.p.z, rot: f.yaw, c: (i / 36) % 3 === 0 ? 0xdff8ff : 0x2ee6ff, s: 1, sy: 1 + ((i / 36) % 2) * 0.5 }); }
  instancedColored(frame, new THREE.MeshBasicMaterial({ color: 0xffffff, toneMapped: false }), gates, false);
}
{ // canyon along the back straight: tall dark walls with vertical light seams and a lit top edge
  ...
  for (const [i0, i1] of CANYON) for (let i = i0; i < i1; i++) for (const side of [1, -1]) {
    ...
    const a = new THREE.Vector3(A.p.x + A.n.x * side * off, 0, A.p.z + A.n.z * side * off), b = new THREE.Vector3(B.p.x + B.n.x * side * off, 0, B.p.z + B.n.z * side * off);
    const v0 = CUM[i] / 8, v1 = (CUM[i] + 3.6) / 8;
    pos.push(a.x, 0, a.z, b.x, 0, b.z, b.x, h, b.z, a.x, 0, a.z, b.x, h, b.z, a.x, h, a.z);
  ...
  const bridges = []; for (let i = CANYON[0][0] + 20; i < CANYON[0][1] - 10; i += 40) { const f = frameAt(i); bridges.push({ x: f.p.x, z: f.p.z, rot: f.yaw }); }
...
function frame(now) {
  ...
  rig.position.copy(car.position); rig.rotation.y = st.theta; studio.position.copy(car.position);
  trailUpdate();
