...
| **Team Fortress 2** | A different model for each of the nine classes. Eight are from the mercenary set; the Demoman is generated by `tools/make-demoman.js`, because that set contains no Demoman. Team colour is a shader repaint of strongly red-dominant cloth, leaving skin, the Medic's white coat and the Sniper's khaki alone. |
...
Attribution (CC BY)**. The Demoman comes from neither set — he is generated by
`tools/make-demoman.js` and is original to this project. See `assets/models/CREDITS.txt` for what was changed; the
