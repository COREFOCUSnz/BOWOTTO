// Frostbite Keep: an original "wild" map. Same proven three-corridor-into-
// a-hub skeleton, geometrically identical to Casbah/Rock2/Ghost Town —
// themed as a frozen keep: pale stone and blue-tinted ice, a frost-locked
// courtyard for a hub.
(function (root) {
  'use strict';
  ...
  function buildMap() {
    ...
    function fort(team) {
      ...
      F([[-5, 0, -27], [-4, 4, -25]], MAT.CONCRETE);   // dividing pillars: no open room
      F([[4, 0, -25], [5, 4, -23]], MAT.CONCRETE);     // over ~8 m stays undivided
      F([[-13, 0, -22], [13, 5, -21.5]], MAT.STONE);    // flag room's own south wall
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -22], [gx1, 4, -21.5]]);   // the three doors
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -21.5], [gx1, 4, -4]]);    // the three frozen passages
      for (const [wx0, wx1] of [[-13, -11], [-7, -2], [2, 7], [11, 13]]) {
        F([[wx0, 0, -21.5], [wx1, 5, -4]], MAT.STONE);  // walls BETWEEN the lanes
      }
      // a snowdrift/ice block at two points down each lane
      F([[-10.5, 0, -14.5], [-9.5, 2.2, -13.5]], MAT.CONCRETE);
      F([[-1.5, 0, -14.5], [-0.5, 2.2, -13.5]], MAT.CONCRETE);
      F([[8, 0, -14.5], [9, 2.2, -13.5]], MAT.CONCRETE);
      F([[-8.5, 0, -7.5], [-7.5, 2.2, -6.5]], MAT.CONCRETE);
      F([[0.5, 0, -7.5], [1.5, 2.2, -6.5]], MAT.CONCRETE);
      F([[9.5, 0, -7.5], [10.5, 2.2, -6.5]], MAT.CONCRETE);

      // Materials — frozen keep, not a bunker
    ...
    // The frost-locked courtyard: shared, built once.
    ...
    W.fill([[-13.5, 6, -4.5], [13.5, 6.5, 4.5]], MAT.CONCRETE);
  ...
  const out = { buildMap };
  if (isNode) module.exports = out; else root.MAP_FROSTBITE = out;
