// Badlands: a TFC-inspired map — the fifth and last of the "based on
// original TFC maps" half of the request (Warpath, Well, Rock2, Casbah,
// Badlands). Same proven three-corridor-into-a-hub skeleton, geometrically
// identical to the reskins that followed it — themed as a sun-scorched
// desert wasteland: sandstone and dirt, a dust-blown crossing for a hub.
(function (root) {
  'use strict';
  ...
  function buildMap() {
    ...
    function fort(team) {
      ...
      F([[-13, 0, -40], [13, 6, -22]], MAT.DIRT);
      C([[-6, 0, -38], [6, 4, -31]]);          // spawn
      C([[-2, 0, -31], [2, 4, -28]]);          // hallway
      C([[-12, 0, -28], [12, 5, -22]]);        // flag room, one wide room with three exits
      F([[-5, 0, -27], [-4, 4, -25]], MAT.STONE);   // dividing pillars: no open room
      F([[4, 0, -25], [5, 4, -23]], MAT.STONE);     // over ~8 m stays undivided
      F([[-13, 0, -22], [13, 5, -21.5]], MAT.DIRT);    // flag room's own south wall
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -22], [gx1, 4, -21.5]]);   // the three doors
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -21.5], [gx1, 4, -4]]);    // the three desert cuts
      for (const [wx0, wx1] of [[-13, -11], [-7, -2], [2, 7], [11, 13]]) {
        F([[wx0, 0, -21.5], [wx1, 5, -4]], MAT.DIRT);  // walls BETWEEN the lanes
      }
      // a boulder/sandbag stack at two points down each lane
      ...
      // Materials — sun-scorched wasteland, not a bunker
    ...
    // The dust-blown crossing: shared, built once.
    for (const z of [-4.5, 4]) W.fill([[-13, 0, z], [13, 7, z + 0.5]], MAT.DIRT);
    for (const [gx0, gx1] of GAPS) {
      W.carve([[gx0, 0, -4.5], [gx1, 4, -4]]);
      W.carve([[gx0, 0, 4], [gx1, 4, 4.5]]);
    }
    W.fill([[-13.5, 0, -4], [-13, 7, 4]], MAT.DIRT);
    W.fill([[13, 0, -4], [13.5, 7, 4]], MAT.DIRT);
    W.fill([[-13.5, 6, -4.5], [13.5, 6.5, 4.5]], MAT.STONE);
    W.carve([[-13, 0, -4], [13, 6, 4]]);
    W.paint([[-13, 0, -4], [13, 6, 4]], MAT.STONE);
  ...
  const out = { buildMap };
  if (isNode) module.exports = out; else root.MAP_BADLANDS = out;
