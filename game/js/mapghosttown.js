// Ghost Town: an original "wild" map. Same proven three-corridor-into-a-hub
// skeleton as Casbah/Rock2 — kept geometrically identical to those two this
// time (Thunderdome's extra decorative geometry measurably nudged its
// capture rate, so no new solid props here) — themed as an abandoned
// frontier town: weathered wood and brick, a dusty town square for a hub.
(function (root) {
  'use strict';
  ...
  function buildMap() {
    ...
    function fort(team) {
      ...
      F([[-13, 0, -40], [13, 6, -22]], MAT.WOOD);
      C([[-6, 0, -38], [6, 4, -31]]);          // spawn
      C([[-2, 0, -31], [2, 4, -28]]);          // hallway
      C([[-12, 0, -28], [12, 5, -22]]);        // flag room, one wide room with three exits
      F([[-5, 0, -27], [-4, 4, -25]], MAT.DARKBRICK);   // dividing pillars: no open room
      F([[4, 0, -25], [5, 4, -23]], MAT.DARKBRICK);     // over ~8 m stays undivided
      F([[-13, 0, -22], [13, 5, -21.5]], MAT.WOOD);    // flag room's own south wall
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -22], [gx1, 4, -21.5]]);   // the three doors
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -21.5], [gx1, 4, -4]]);    // the three back streets
      for (const [wx0, wx1] of [[-13, -11], [-7, -2], [2, 7], [11, 13]]) {
        F([[wx0, 0, -21.5], [wx1, 5, -4]], MAT.WOOD);  // walls BETWEEN the lanes
      }
      // a wagon/barrel stack at two points down each lane
      F([[-10.5, 0, -14.5], [-9.5, 2.2, -13.5]], MAT.DARKBRICK);
      F([[-1.5, 0, -14.5], [-0.5, 2.2, -13.5]], MAT.DARKBRICK);
      F([[8, 0, -14.5], [9, 2.2, -13.5]], MAT.DARKBRICK);
      F([[-8.5, 0, -7.5], [-7.5, 2.2, -6.5]], MAT.DARKBRICK);
      F([[0.5, 0, -7.5], [1.5, 2.2, -6.5]], MAT.DARKBRICK);
      F([[9.5, 0, -7.5], [10.5, 2.2, -6.5]], MAT.DARKBRICK);

      // Materials — weathered frontier town
      ...
      for (const [gx0, gx1] of GAPS) PAINT([[gx0, 0, -21.5], [gx1, 4, -4]], MAT.DIRT);
      F([[-13, 5.5, -40], [13, 6, -22]], MAT.WOOD);
    ...
    // The town square: shared, built once.
    for (const z of [-4.5, 4]) W.fill([[-13, 0, z], [13, 7, z + 0.5]], MAT.WOOD);
    for (const [gx0, gx1] of GAPS) {
      W.carve([[gx0, 0, -4.5], [gx1, 4, -4]]);
      W.carve([[gx0, 0, 4], [gx1, 4, 4.5]]);
    }
    W.fill([[-13.5, 0, -4], [-13, 7, 4]], MAT.WOOD);
    W.fill([[13, 0, -4], [13.5, 7, 4]], MAT.WOOD);
    W.fill([[-13.5, 6, -4.5], [13.5, 6.5, 4.5]], MAT.DARKBRICK);
    W.carve([[-13, 0, -4], [13, 6, 4]]);
    W.paint([[-13, 0, -4], [13, 6, 4]], MAT.DIRT);
    W.fill([[-5, 0, -2], [-3.5, 5, -0.5]], MAT.DARKBRICK);
    W.fill([[3.5, 0, 0.5], [5, 5, 2]], MAT.DARKBRICK);
    W.fill([[-10, 0, -2], [-8.5, 5, -0.5]], MAT.DARKBRICK);
    W.fill([[-10, 0, 0.5], [-8.5, 5, 2]], MAT.DARKBRICK);
    W.fill([[8.5, 0, -2], [10, 5, -0.5]], MAT.DARKBRICK);
    W.fill([[8.5, 0, 0.5], [10, 5, 2]], MAT.DARKBRICK);
  ...
  const out = { buildMap };
  if (isNode) module.exports = out; else root.MAP_GHOSTTOWN = out;
