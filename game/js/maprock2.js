// Rock2: a TFC-inspired canyon map. Same three-corridor-into-a-hub skeleton
// as Well — proven this time, not guessed at: real dividing walls between
// the three lanes (not just doors lining up with matching gaps, which is
// all that separated them at first on Well and left the whole stretch one
// open field), a cover block at two points down each lane so no lane is an
// uninterrupted shooting gallery, and no open room over ~8 m undivided.
// Themed as a rock canyon (stone/dirt, not brick) instead of Well's desert
// base.
(function (root) {
  'use strict';
  ...
  function buildMap() {
    const world = new World([-20, -6, -46], [20, 12, 46], 0.5);
    const W = world;
    W.fill([[-20, -6, -46], [20, 0, 46]], MAT.DIRT);
    W.fill([[-20, 0, -46], [-18, 10, 46]], MAT.STONE);
    W.fill([[18, 0, -46], [20, 10, 46]], MAT.STONE);
    W.fill([[-20, 0, -46], [20, 10, -42]], MAT.STONE);
    W.fill([[-20, 0, 42], [20, 10, 46]], MAT.STONE);
    ...
    function fort(team) {
      ...
      F([[-5, 0, -27], [-4, 4, -25]], MAT.DARKBRICK);   // dividing pillars: no open
      F([[4, 0, -25], [5, 4, -23]], MAT.DARKBRICK);     // room over ~8 m stays undivided
      F([[-13, 0, -22], [13, 5, -21.5]], MAT.STONE);    // flag room's own south wall
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -22], [gx1, 4, -21.5]]);   // the three doors
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -21.5], [gx1, 4, -4]]);    // the three canyon corridors
      for (const [wx0, wx1] of [[-13, -11], [-7, -2], [2, 7], [11, 13]]) {
        F([[wx0, 0, -21.5], [wx1, 5, -4]], MAT.STONE);  // walls BETWEEN the lanes
      }
      // a cover block at two points down each lane
  ...
  const out = { buildMap };
  if (isNode) module.exports = out; else root.MAP_ROCK2 = out;
