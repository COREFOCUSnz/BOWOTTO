// Scrapyard Siege: an original "wild" map, and the tenth and last. Same
// proven three-corridor-into-a-hub skeleton, geometrically identical to
// Casbah/Rock2/Ghost Town/Frostbite/Neon Sprawl — themed as a junkyard
// under siege: rust-dark brick and scrap metal, a compactor yard for a hub.
(function (root) {
  'use strict';
  ...
  function buildMap() {
    ...
    function fort(team) {
      ...
      F([[-13, 0, -40], [13, 6, -22]], MAT.DARKBRICK);
      ...
      F([[-13, 0, -22], [13, 5, -21.5]], MAT.DARKBRICK);    // flag room's own south wall
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -22], [gx1, 4, -21.5]]);   // the three doors
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -21.5], [gx1, 4, -4]]);    // the three scrap lanes
      for (const [wx0, wx1] of [[-13, -11], [-7, -2], [2, 7], [11, 13]]) {
        F([[wx0, 0, -21.5], [wx1, 5, -4]], MAT.DARKBRICK);  // walls BETWEEN the lanes
      }
      // a car-crusher stack at two points down each lane
      ...
      // Materials — rusted junkyard, not a bunker
    ...
    // The compactor yard: shared, built once.
    for (const z of [-4.5, 4]) W.fill([[-13, 0, z], [13, 7, z + 0.5]], MAT.DARKBRICK);
    for (const [gx0, gx1] of GAPS) {
      W.carve([[gx0, 0, -4.5], [gx1, 4, -4]]);
      W.carve([[gx0, 0, 4], [gx1, 4, 4.5]]);
    }
    W.fill([[-13.5, 0, -4], [-13, 7, 4]], MAT.DARKBRICK);
    W.fill([[13, 0, -4], [13.5, 7, 4]], MAT.DARKBRICK);
  ...
  const out = { buildMap };
  if (isNode) module.exports = out; else root.MAP_SCRAPYARD = out;
