// Casbah: a TFC-inspired market-fortress map. Same proven three-corridor-
// into-a-hub skeleton as Well/Rock2 (real dividing walls between lanes, a
// cover block at two points down each one, no open room over ~8 m
// undivided) — themed as a walled bazaar: brick and wood instead of
// Rock2's stone canyon, the hub dressed as a market square with crate
// stalls instead of bare pillars.
(function (root) {
  'use strict';
  ...
  function buildMap() {
    ...
    function fort(team) {
      ...
      F([[-13, 0, -40], [13, 6, -22]], MAT.BRICK);
      C([[-6, 0, -38], [6, 4, -31]]);          // spawn
      C([[-2, 0, -31], [2, 4, -28]]);          // hallway
      C([[-12, 0, -28], [12, 5, -22]]);        // flag room, one wide room with three exits
      F([[-5, 0, -27], [-4, 4, -25]], MAT.WOOD);   // dividing pillars: no open room
      F([[4, 0, -25], [5, 4, -23]], MAT.WOOD);     // over ~8 m stays undivided
      F([[-13, 0, -22], [13, 5, -21.5]], MAT.BRICK);    // flag room's own south wall
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -22], [gx1, 4, -21.5]]);   // the three doors
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -21.5], [gx1, 4, -4]]);    // the three market lanes
      for (const [wx0, wx1] of [[-13, -11], [-7, -2], [2, 7], [11, 13]]) {
        F([[wx0, 0, -21.5], [wx1, 5, -4]], MAT.BRICK);  // walls BETWEEN the lanes
      }
      // a crate stall at two points down each lane
      F([[-10.5, 0, -14.5], [-9.5, 2.2, -13.5]], MAT.WOOD);
      F([[-1.5, 0, -14.5], [-0.5, 2.2, -13.5]], MAT.WOOD);
      F([[8, 0, -14.5], [9, 2.2, -13.5]], MAT.WOOD);
      F([[-8.5, 0, -7.5], [-7.5, 2.2, -6.5]], MAT.WOOD);
      F([[0.5, 0, -7.5], [1.5, 2.2, -6.5]], MAT.WOOD);
      F([[9.5, 0, -7.5], [10.5, 2.2, -6.5]], MAT.WOOD);

      // Materials — walled bazaar, not a bunker
    ...
    // Central market square: shared, built once.
    for (const z of [-4.5, 4]) W.fill([[-13, 0, z], [13, 7, z + 0.5]], MAT.BRICK);
    for (const [gx0, gx1] of GAPS) {
      W.carve([[gx0, 0, -4.5], [gx1, 4, -4]]);
      W.carve([[gx0, 0, 4], [gx1, 4, 4.5]]);
    }
    W.fill([[-13.5, 0, -4], [-13, 7, 4]], MAT.BRICK);
    W.fill([[13, 0, -4], [13.5, 7, 4]], MAT.BRICK);
    W.fill([[-13.5, 6, -4.5], [13.5, 6.5, 4.5]], MAT.WOOD);
    W.carve([[-13, 0, -4], [13, 6, 4]]);
    W.paint([[-13, 0, -4], [13, 6, 4]], MAT.DARKBRICK);
    W.fill([[-5, 0, -2], [-3.5, 5, -0.5]], MAT.WOOD);
    W.fill([[3.5, 0, 0.5], [5, 5, 2]], MAT.WOOD);
    W.fill([[-10, 0, -2], [-8.5, 5, -0.5]], MAT.WOOD);
    W.fill([[-10, 0, 0.5], [-8.5, 5, 2]], MAT.WOOD);
    W.fill([[8.5, 0, -2], [10, 5, -0.5]], MAT.WOOD);
    W.fill([[8.5, 0, 0.5], [10, 5, 2]], MAT.WOOD);
  ...
  const out = { buildMap };
  if (isNode) module.exports = out; else root.MAP_CASBAH = out;
