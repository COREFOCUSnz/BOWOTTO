// Neon Sprawl: an original "wild" map. Same proven three-corridor-into-a-hub
// skeleton, geometrically identical to Casbah/Rock2/Ghost Town/Frostbite —
// themed as a cyberpunk back-alley sprawl: metal and grating under glaring
// light, a neon-lit plaza for a hub.
(function (root) {
  'use strict';
  ...
  function buildMap() {
    ...
    const W = world;
    W.fill([[-20, -6, -44], [20, 0, 44]], MAT.CONCRETE);
    W.fill([[-20, 0, -44], [-18, 10, 44]], MAT.METAL);
    W.fill([[18, 0, -44], [20, 10, 44]], MAT.METAL);
    W.fill([[-20, 0, -44], [20, 10, -40]], MAT.METAL);
    W.fill([[-20, 0, 40], [20, 10, 44]], MAT.METAL);
    ...
    function fort(team) {
      ...
      F([[-13, 0, -22], [13, 5, -21.5]], MAT.METAL);    // flag room's own south wall
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -22], [gx1, 4, -21.5]]);   // the three doors
      for (const [gx0, gx1] of GAPS) C([[gx0, 0, -21.5], [gx1, 4, -4]]);    // the three back alleys
      for (const [wx0, wx1] of [[-13, -11], [-7, -2], [2, 7], [11, 13]]) {
        F([[wx0, 0, -21.5], [wx1, 5, -4]], MAT.METAL);  // walls BETWEEN the lanes
      }
      // a dumpster/vending unit at two points down each lane
      ...
      // Materials — cyberpunk sprawl, not a bunker
    ...
    // The neon plaza: shared, built once.
    for (const z of [-4.5, 4]) W.fill([[-13, 0, z], [13, 7, z + 0.5]], MAT.METAL);
    for (const [gx0, gx1] of GAPS) {
      W.carve([[gx0, 0, -4.5], [gx1, 4, -4]]);
      W.carve([[gx0, 0, 4], [gx1, 4, 4.5]]);
    }
    W.fill([[-13.5, 0, -4], [-13, 7, 4]], MAT.METAL);
    W.fill([[13, 0, -4], [13.5, 7, 4]], MAT.METAL);
  ...
  const out = { buildMap };
  if (isNode) module.exports = out; else root.MAP_NEONSPRAWL = out;
